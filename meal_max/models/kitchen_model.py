...
def create_meal(meal: str, cuisine: str, price: float, difficulty: str) -> None:
    ...
    Args:
        ...
        difficulty (str): The difficulty of the meal, where the value in ['LOW', 'MED', 'HIGH'].
...
def delete_meal(meal_id: int) -> None:
    """
    Soft deletes a meal from the meals by marking it as deleted.
...
def get_leaderboard(sort_by: str="wins") -> dict[str, Any]:
    """
    Build leaderboard that the meal are not marked as deleted from the meals.

    Args:
        sort_by (str): If wins, sort the meals by wins count in descending order.
                        else win_pct, sort the meals by (wins * 1.0 / battles) AS win_pct

    Returns:
        list[dict]: A list of dictionaries representing all non-deleted meals with wins and win_pct.

    Raises:
        ValueError: If the sort_by with the invalid value does not in ['wins', 'win_pct'].
        sqlite3.Error: If any database error occurs.
    """
    query = """
...
def get_meal_by_id(meal_id: int) -> Meal:
    """
    Retrieves a meal from the meals by its meal ID.
...
def get_meal_by_name(meal_name: str) -> Meal:
    """
    Retrieves a meal from the meals by its meal name.
